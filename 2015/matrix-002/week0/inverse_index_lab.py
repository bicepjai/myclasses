...
def makeInverseIndex(strlist):
    ...
    True
    """
    setOfWords = { word for line in strlist for word in line.split() }
    dic = {}
    for word in setOfWords:
        dic[word] = {i for (i,line) in enumerate(strlist) if word in line.split()}
    return dic
...
def orSearch(inverseIndex, query):
    ...
    s =set()
    for qword in query:
        if qword in inverseIndex.keys():
            s = s | inverseIndex[qword]
    return s
