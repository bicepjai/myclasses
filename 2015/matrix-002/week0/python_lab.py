...
S_intersect_T = { x for x in S for y in T if (y == x) }
...
odd_num_list_range = {x+1 for x in range(0,100,2)}
...
list_sum_zip = [sum([a,b]) for a,b in list(zip(A,B))]
...
representation_dict = { e[0]:e[1] for e in list(zip(range(base**3),[ (x,y,z) for x in digits for y in digits for z in digits])) }
