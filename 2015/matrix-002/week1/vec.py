...
def equal(u,v):
    ...
    """
    assert u.D == v.D
    if u.f.items() == v.f.items():
        return True
    else:
        uf = dict(list(u.f.items()) + [(k,0) for k in [x for x in u.D if x not in u.f.keys()]] )
        vf = dict(list(v.f.items()) + [(k,0) for k in [x for x in v.D if x not in v.f.keys()]] )
        if uf.items() == vf.items():
            return True
        return False
...
def add(u,v):
    ...
    """
    assert u.D == v.D
    vf = { k:x for k,x in v.f.items() if k in v.D}
    uf = { k:x for k,x in u.f.items() if k in u.D}

    f = dict(list(vf.items()) + list(uf.items()))
    for k in [x for x in uf.keys() if x in vf.keys()]:
        f[k] = uf[k] + vf[k]
    for k in v.D:
        if k not in f.keys():
            f[k] = 0
...
def dot(u,v):
    ...
    """
    assert u.D == v.D
    return sum([u.f[k] * v.f[k] for k in u.f.keys() if k in v.f.keys()])
