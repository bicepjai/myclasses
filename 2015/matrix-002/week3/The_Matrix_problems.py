...
# For example, the 2x2 identity matrix would [[0,1],[1,0]].
...
def lin_comb_mat_vec_mult(M, v):
    ...
    '''
    assert (M.D[1] == v.D)
    return sum( [v[col] * Vec(M.D[0], {row:M[row,col] for row in M.D[0]}) for col in M.D[1] ])
...
def lin_comb_vec_mat_mult(v, M):
    ...
    '''
    assert(v.D == M.D[0])
    return sum([ v[row] * Vec(M.D[1], {col:M[row,col] for col in M.D[1]}) for row in M.D[0] ])
...
def dot_product_mat_vec_mult(M, v):
    ...
    '''
    assert(M.D[1] == v.D)
    return Vec(M.D[0], {row: Vec(M.D[1], {col:M[row,col] for col in M.D[1]}) * v for row in M.D[0]})
...
def dot_product_vec_mat_mult(v, M):
    ...
    Examples:
      ...
      '''
    assert(v.D == M.D[0])
    return Vec(M.D[1], {col: Vec(M.D[0], {row:M[row,col] for row in M.D[0]}) * v for col in M.D[1]})
...
def Mv_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    colDicts = mat2coldict(B)
    f = {}
    for colAB,colV in colDicts.items():
        v = A*colV
        for rowAB in A.D[0]:
            f[rowAB,colAB] = v[rowAB]
...
def vM_mat_mat_mult(A, B):
    ...
    f = {}
    for rowAB,rowV in rowDicts.items():
        v = rowV*B
        for colAB in B.D[1]:
            f[rowAB,colAB] = v[colAB]
    return Mat((A.D[0], B.D[1]), f)
    # return Mat((A.D[0], B.D[1]), {(rowAB,colAB):(rowV*B)[colAB] for colAB in B.D[1] for rowAB,rowV in mat2rowdict(A).items() })
...
x1 = solve(rowdict2mat(button_vectors(9)),b1)

#residual
r1 = b1 - (rowdict2mat(button_vectors(9)) * x1)

#Is x1 really a solution? Assign True if yes, False if no.
is_good1 = rowdict2mat(button_vectors(9)) * x1 == b1
...
#Solution given by solver
x2 = solve(rowdict2mat(button_vectors(9)),b2)

#residual
r2 = b2 - (rowdict2mat(button_vectors(9)) * x2)

#Is it really a solution? Assign True if yes, False if no.
is_good2 = rowdict2mat(button_vectors(9)) * x2 == b2
