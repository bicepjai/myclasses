...
def equal(u,v):
    ...
    if u.f.items() == v.f.items():
        ...
        diffKeys = set(u.f.keys() | v.f.keys()) - set(u.f.keys() & v.f.keys())
        uf = dict(list(u.f.items()) + [(k,0) for k in diffKeys if k not in u.f.keys()] )
        vf = dict(list(v.f.items()) + [(k,0) for k in diffKeys if k not in v.f.keys()] )
...
def add(u,v):
    ...
    uvfKeys = set(u.f.keys() & v.f.keys())
    f = dict(list(v.f.items()) + list(u.f.items()))
    for k in uvfKeys:
        f[k] = u.f[k] + v.f[k]
    # for k in v.D:
    #     if k not in f.keys():
