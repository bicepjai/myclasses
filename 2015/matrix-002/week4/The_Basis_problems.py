...
indep_vec_1 = Vec({0,1,2}, {0:1,1:0,2:1,3:0})
indep_vec_2 = Vec({0,1,2}, {0:1,1:1,2:0,3:0})
indep_vec_3 = Vec({0,1,2}, {0:0,1:0,2:1,3:1})
indep_vec_4 = Vec({0,1,2}, {0:0,1:1,2:0,3:10})
...
def is_superfluous(S, v):
    ...
    if len(S) == 0:
        return False
    if len(S) == 1:
        if S.copy().pop().is_almost_zero():
            return True
    solution_v = coldict2mat(list(S-{v})) * vec2rep(list(S-{v}),v)
    if v == solution_v or (v-solution_v).is_almost_zero() :
        return True
    return False
...
def exchange(S, A, z):
    ...
    for w in (listSA):
        ...
        if w_coeff != 0:
            dw = (1/w_coeff) * (z - sum([ v2r[[k for k,u in colDict.items() if u == v][0]] * v for v in S - {w}]))
