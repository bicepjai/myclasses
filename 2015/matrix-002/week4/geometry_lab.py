...
def grayscale():
    ...
    '''
    rgb = {'r':77/256, 'g':151/256, 'b':28/256}
    labels = {'r','g','b'}
    return Mat((labels,labels), {(a,b):rgb[b] for b in labels for a in labels })
...
def reflect_about(x1, y1, x2, y2):
    ...
    '''
    if (x2 - x1) == 0:
        return translation(0,x1) * reflect_y() * translation(0,-x1)
    m = (y2 - y1) / (x2 - x1)
    b = y1 - m * x1
    angle = math.atan(m)
    return translation(0,b) * rotation(2 * angle) * reflect_x() * translation(0,-b)
    # return translation(0,b) * rotation(-(2 * angle)) * reflect_y() * translation(0,-b)
