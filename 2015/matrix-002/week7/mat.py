...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    for item in A.f.items() | B.f.items() :
        if item not in A.f.items() or item not in B.f.items():
            if item[1] != 0:
                return False
    return A.D == B.D
...
def setitem(M, k, val):
    ...
    if k in M.f.keys():
        M.f[k] = val
    elif k[0] in M.D[0] and k[1] in M.D[1]:
        M.f[k] = val
    else:
        M.f[k] = 0
...
def add(A, B):
    ...
    """
    assert A.D == B.D
    M = Mat((A.D[0].copy(),A.D[1].copy()),A.f.copy())
...
def vector_matrix_mul(v, M):
    ...
    """
    assert M.D[0] == v.D
    sparseR = {k[0] for k,_ in M.f.items()}
    sparseC = {k[1] for k,_ in M.f.items()} 
    vM = Vec(M.D[1], {})
    for row in sparseR:
        vM = vM + v[row] * Vec(M.D[1], {col:M[row,col] for col in sparseC })
    return vM
...
def matrix_vector_mul(M, v):
    ...
    Mv = Vec(M.D[0],{})
    for col in sparseC:
        Mv = Mv + v[col] * Vec(M.D[0], {row:M[row,col] for row in sparseR })
    return Mv
...
def matrix_matrix_mul(A, B):
    ...
    assert A.D[1] == B.D[0]
    f = {}
    sparseR  = {k[0] for k,_ in A.f.items()}
    sparseCR = {k[1] for k,_ in A.f.items()}
    sparseC  = {k[1] for k,_ in B.f.items()} 
    for rowA in sparseR:
        for colB in sparseC:
            f[rowA, colB] = sum([A[rowA,colAB]*B[colAB,colB] for colAB in sparseCR])
