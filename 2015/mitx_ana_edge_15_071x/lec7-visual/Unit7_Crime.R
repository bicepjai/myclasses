...
LatLonCountsFreq = subset(LatLonCounts, Freq > 0)

# Plot these points on our map:
ggmap(chicago) + geom_point(data = LatLonCountsFreq, aes(x = Long, y = Lat, color = Freq, size=Freq))

# Change the color scheme:
ggmap(chicago) + geom_point(data = LatLonCountsFreq, aes(x = Long, y = Lat, color = Freq, size=Freq)) + scale_colour_gradient(low="yellow", high="red")

# We can also use the geom_tile geometry
ggmap(chicago) + geom_tile(data = LatLonCountsFreq, aes(x = Long, y = Lat, alpha = Freq), fill="red")
...
ggplot(murderMap, aes(x = long, y = lat, group = group, fill = GunOwnership)) + geom_polygon(color = "black") + scale_fill_gradient(low = "black", high = "red", guide = "legend", limits = c(0,0.6))
