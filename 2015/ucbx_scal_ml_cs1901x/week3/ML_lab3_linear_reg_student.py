...
def parsePoint(line):
    ...
    Returns:
        LabeledPoint: The line is converted into a `LabeledPoint`, which consists of a label and
            features.
    """
    col = filter(None, line.strip().split(","))
    return LabeledPoint(col[0], col[1:])

parsedSamplePoints = [parsePoint(samplePoints[x]) for x in xrange(0, len(samplePoints))]
...
def twoWayInteractions(lp):
    ...
    """
    return LabeledPoint(lp.label, np.hstack((lp.features,map(np.prod,itertools.product(lp.features, repeat=2)))))
