...
```
```{r sfo-feb-flights-arrival-delays-plot}
...
sfo_feb_flights %>% group_by(carrier) %>% summarise(iqr=IQR(arr_delay))
```
...
summarize(on_time_proportion=sum(arr_type=="on time")/n())
```
...
nycflights %>% 
  group_by(dep_type) %>% 
  summarise(delayed_dep_ontime_arr = sum(arr_type == "on time")/n())
