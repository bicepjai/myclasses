...
```{r samp1-area-stats}
samp1 %>% summarise(mu = mean(area), sigma = sd(area))
```
...
```{r sampling-dist}
sample_means50 %>% summarise(mu = mean(x_bar), pop_med = median(x_bar), 
...
sample_means_small %>% summarise(mu = mean(x_bar), pop_med = median(x_bar), 
            sigma = sd(x_bar), pop_iqr = IQR(x_bar),
            pop_min = min(x_bar), pop_max = max(x_bar),
            pop_q1 = quantile(x_bar, 0.25),  # first quartile, 25th percentile
            pop_q3 = quantile(x_bar, 0.75),
...
```{r price-sample}
ames %>% sample_n(size = 50) %>% summarize(x_bar=mean(area))
```
...
```{r price-sampling}
sample_means50 <- ames %>% 
                  rep_sample_n(size=50, rep=5000, replace=TRUE) %>% 
...
sample_means50 <- ames %>% 
                  rep_sample_n(size=150, rep=5000, replace=TRUE) %>% 
                  summarise(x_bar = mean(price))
ggplot(data=sample_means50, aes(x=x_bar)) + geom_histogram()
...
sample_mean15 <- ames %>% rep_sample_n(size=15, rep=2000, replace=TRUE) %>% 
         summarise(x_bar=mean(price))
ggplot(data=sample_mean15, aes(x=x_bar)) + geom_histogram()
...
sample_mean150 <- ames %>% rep_sample_n(size=150, rep=2000, replace=TRUE) %>% 
         summarise(x_bar=mean(price))
ggplot(data=sample_mean15, aes(x=x_bar)) + geom_histogram()
```
