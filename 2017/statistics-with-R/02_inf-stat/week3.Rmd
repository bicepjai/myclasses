...
```{r summary}
summary(nc$gained)
nc %>% filter(is.na(gained)) %>% summarise(n=n())
...
```{r weeks-ci-90}
inference(y = weeks, data = nc, statistic = "mean", type = "ci", 
          conf_level=0.90, method = "theoretical", order = c("smoker","nonsmoker"))
```
...
inference(y = weeks, x = mature, data = nc, statistic = "mean", type = "ht", null = 0,
          alternative = "twosided", method = "theoretical", order = c("mature","younger"))
```
...
nc  %>% group_by(data=mature) %>% summarize(min=min(mage), max=max(mage))
