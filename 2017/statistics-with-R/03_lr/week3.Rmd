...
evals %>% filter(score <= 3) %>% summarize(n=n())
summary(evals$score)
ggplot(data=evals, aes(x=score)) + geom_histogram()
ggplot(data=evals, aes(x="score", y=score)) + geom_boxplot()
...
ggplot(data=evals, aes(y=score, x=pic_outfit)) + geom_boxplot()
ggplot(data=evals, aes(y=score, x=pic_color)) + geom_boxplot()
ggplot(data=evals, aes(y=score, x=language)) + geom_boxplot()
ggplot(data=evals, aes(y=score, x=cls_level)) + geom_boxplot()
ggplot(data=evals, aes(y=score, x=age)) + geom_point()
ggplot(data=evals, aes(y=score, x=bty_avg)) + geom_point()
...
newprof_f <- data.frame(gender = "female", bty_avg = 3)
newprof
```
...
```{r p-val-select}
# type your code for the Exercise here, and Knit
m_val1 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
