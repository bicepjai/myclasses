...
wage %>% filter(wage <= 300) %>% summarize(n=n())
summary(wage$wage)
```
...
```{r two-vars-eda2}
# type your code for the Exercise here, and Knit
ggplot(data=wage, aes(x = as.factor(age), y = wage)) +
    geom_boxplot()
```
...
```{r two-vars-eda6}
# type your code for the Exercise here, and Knit
ggplot(data=wage, aes(x = as.factor(exper), y = wage)) +
    geom_point()
```

```{r two-vars-eda7}
# type your code for the Exercise here, and Knit
ggplot(data=wage, aes(x = black, y = wage)) +
    geom_boxplot()
```
...
plot(m_wage_iq$residuals, m_wage_iq$fitted.values)
...
summary(m_wage_educ)
plot(m_wage_educ$residuals, m_wage_educ$fitted.values)
...
```{r log-resid}
summary(m_lwage_iq)
plot(m_lwage_iq$residuals, m_lwage_iq$fitted.values)
...
```{r stepBIC}
stepAIC(lm(lwage ~ . -wage, data = na.omit(wage)),
        k=log(nrow(na.omit(wage)))
        )
...
```{r}
wage_all_no_na = na.omit(wage)
bma_lwage_all = bas.lm(lwage ~ . -wage, data = wage_all_no_na,
                   prior ="ZS-null", 
                   modelprior = beta.binomial(1,1))
bma_lwage_all
summary(bma_lwage_all)
```
...
```{r graph}
par(mfrow = c(1,1))
coef_lwage = coefficients(bma_lwage_all)
coef_lwage
...
```{r tplot}
curve(dt((3 / 5) * (x - 1), df = 3), from = -10, to = 10)
```
